/**
 * Script de importação do Diário Oficial
 * node importar-diario.js
 */
...
async function listarArquivos(pastaId, pageToken = null) {
  let url = `${DRIVE_API_BASE}/files?q='${pastaId}'+in+parents&key=${DRIVE_API_KEY}&fields=files(id,name,mimeType),nextPageToken&pageSize=1000`
...
async function main() {
      ...
      for (const pdf of pdfs) {
        try {
          ...
          const [existente] = await conn.execute(
            'SELECT id FROM diario_oficial WHERE url LIKE ?', [`%${pdf.id}%`]
          )
          if (existente.length > 0) { console.log(`   ⏭  Já existe: ${pdf.name}`); continue }
  ...
  console.log(`✅ Concluído! Inseridos: ${totalInseridos} | Erros: ${totalErros}`)
}
