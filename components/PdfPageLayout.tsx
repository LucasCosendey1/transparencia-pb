...
setLinhasPorTabela(novasLinhas)
  ...
  const gerarPDF = async (linhasOverride?: Linha[]): Promise<string> => {
    ...
    for (const bloco of blocosPdf) {
      ...
      if (bloco.tipo === 'tabela') {
        ...
        if (!meta) continue
        const linhasBloco = linhasOverride ?? (await (await fetch(`/api/linhas/${paginaId}?nome_tabela=${bloco.nome_tabela}`)).json())
  ...
  const baixar = async () => {
    ...
    if (formatoDownload === 'pdf') {
      // Se fonte é um PDF específico
      const pdfEscolhido = pdfsSalvos.find(p => p.nome_pdf === fonteDownload)
      if (pdfEscolhido) {
        const a = document.createElement('a')
        a.href = `data:application/pdf;base64,${pdfEscolhido.pdf_base64}`
        a.download = `${pdfEscolhido.nome_pdf}.pdf`
        a.click()
        return
      }
      // Gera PDF com dados filtrados
      const b64 = await gerarPDF(linhasFiltradas)
      const a = document.createElement('a')
      a.href = `data:application/pdf;base64,${b64}`
      a.download = `${tituloPdf || titulo}.pdf`
      a.click()
      return
    }
                      ...
                      <div className="flex">
                        ...
                        <div className="relative">
                          <button onClick={() => setDropdownFonte(v => !v)}
                            ...
                            {fonteDownload === 'tudo' ? 'Tudo' : fonteDownload} <FaChevronDown size={8} />
                          </button>
                          {dropdownFonte && (
                            <div className="absolute top-full left-0 mt-1 bg-white border rounded-lg shadow-lg z-20 min-w-[140px]">
                              <button onClick={() => { setFonteDownload('tudo'); setDropdownFonte(false) }}
                                className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${fonteDownload === 'tudo' ? 'text-blue-600 font-medium' : 'text-black'}`}>
                                Tudo
                              </button>
                              {tabelas.map(t => (
                                <button key={t.nome_tabela} onClick={() => { setFonteDownload(t.nome_tabela); setDropdownFonte(false) }}
                                  className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${fonteDownload === t.nome_tabela ? 'text-blue-600 font-medium' : 'text-black'}`}>
                                  📊 {t.nome_tabela}
                                </button>
                              ))}
                              {pdfsSalvos.map(p => (
                                <button key={p.nome_pdf} onClick={() => { setFonteDownload(p.nome_pdf); setDropdownFonte(false) }}
                                  className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${fonteDownload === p.nome_pdf ? 'text-blue-600 font-medium' : 'text-black'}`}>
                                  📄 {p.nome_pdf}
                                </button>
                              ))}
                            </div>
